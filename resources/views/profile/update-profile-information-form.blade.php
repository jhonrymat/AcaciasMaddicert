<x-form-section submit="updateProfileInformation">
    ...
    <x-slot name="form">
        ...
        <div class="col-span-6 sm:col-span-4">
            ...
            <x-input id="nombre_2" type="text" class="mt-1 block w-full" wire:model.defer="state.nombre_2" autocomplete="nombre_2" />
            <x-input-error for="nombre_2" class="mt-2" />
        </div>
        {{-- apellido_1 --}}
        <div class="col-span-6 sm:col-span-4">
            <x-label for="apellido_1" value="{{ __('Apellido 1') }}" />
            <x-input id="apellido_1" type="text" class="mt-1 block w-full" wire:model.defer="state.apellido_1" autocomplete="apellido_1" />
            <x-input-error for="apellido_1" class="mt-2" />
        </div>
        {{-- apellido_2 --}}
        <div class="col-span-6 sm:col-span-4">
            <x-label for="apellido_2" value="{{ __('Apellido 2') }}" />
            <x-input id="apellido_2" type="text" class="mt-1 block w-full" wire:model.defer="state.apellido_2" autocomplete="apellido_2" />
        ...
        <div class="col-span-6 sm:col-span-4">
            ...
            <x-input id="telefonoContacto" type="text" class="mt-1 block w-full" wire:model.defer="state.telefonoContacto" autocomplete="telefonoContacto" />
        ...
        {{-- id_tipoSolicitante --}}
        <div class="col-span-6 sm:col-span-4">
            <label for="id_tipoSolicitante" class="block text-sm font-medium text-gray-700">Tipo de Solicitante</label>
            <select id="id_tipoSolicitante" name="id_tipoSolicitante" class="mt-1 block w-full" wire:model.defer="state.id_tipoSolicitante">
                <option value="">-- Selecciona un solicitante --</option>
                @foreach(\App\Models\Tsolicitante::all() as $solicitante)
                    <option value="{{ $solicitante->id }}" {{ old('id_tipoSolicitante', $this->user->id_tipoSolicitante) == $solicitante->id ? 'selected' : '' }}>
                        {{ $solicitante->tipoSolicitante }}
                    </option>
                @endforeach
            </select>
            @error('id_tipoSolicitante') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
        </div>
        {{-- id_tipoDocumento --}}
        <div class="col-span-6 sm:col-span-4">
            ...
            <select wire:model="state.id_tipoDocumento" id="id_tipoDocumento" class="mt-1 block w-full">
                ...
                @foreach(\App\Models\Tdocumento::all() as $tipoDocumento)
                    <option value="{{ $tipoDocumento->id }}" {{ old('id_tipoDocumento', $this->user->id_tipoDocumento) == $tipoDocumento->id ? 'selected' : '' }}>
                        {{ $tipoDocumento->tipoDocumento }}
                    </option>
                @endforeach
            </select>
            @error('id_tipoDocumento') <span class="text-red-500">{{ $message }}</span> @enderror
        </div>
        {{-- numeroIdentificacion --}}
        <div class="col-span-6 sm:col-span-4">
            <x-label for="numeroIdentificacion" value="{{ __('Numero de Identificación') }}" />
            <x-input id="numeroIdentificacion" type="text" class="mt-1 block w-full" wire:model.defer="state.numeroIdentificacion" autocomplete="numeroIdentificacion" />
            <x-input-error for="numeroIdentificacion" class="mt-2" />
        </div>
        {{-- ciudadExpedicion --}}
        <div class="col-span-6 sm:col-span-4">
            <x-label for="ciudadExpedicion" value="{{ __('Ciudad de Expedición') }}" />
            <x-input id="ciudadExpedicion" type="text" class="mt-1 block w-full" wire:model.defer="state.ciudadExpedicion" autocomplete="ciudadExpedicion" />
        ...
        <div class="col-span-6 sm:col-span-4">
            ...
            <select wire:model="state.id_nivelEstudio" id="id_nivelEstudio" class="mt-1 block w-full">
                ...
                @foreach(\App\Models\Nestudio::all() as $nivelEstudio)
                    <option value="{{ $nivelEstudio->id }}" {{ old('id_nivelEstudio', $this->user->id_nivelEstudio) == $nivelEstudio->id ? 'selected' : '' }}>
                        {{ $nivelEstudio->nivelEstudio }}
                    </option>
                @endforeach
            </select>
            @error('id_nivelEstudio') <span class="text-red-500">{{ $message }}</span> @enderror
        </div>
        {{-- id_genero --}}
        <div class="col-span-6 sm:col-span-4">
            ...
            <select wire:model="state.id_genero" id="id_genero" class="mt-1 block w-full">
                ...
                @foreach(\App\Models\Genero::all() as $genero)
                    <option value="{{ $genero->id }}" {{ old('id_genero', $this->user->id_genero) == $genero->id ? 'selected' : '' }}>
                        {{ $genero->nombreGenero }}
                    </option>
                @endforeach
            </select>
            @error('id_genero') <span class="text-red-500">{{ $message }}</span> @enderror
        </div>
        {{-- id_ocupacion --}}
        <div class="col-span-6 sm:col-span-4">
            ...
            <select wire:model="state.id_ocupacion" id="id_ocupacion" class="mt-1 block w-full">
                ...
                @foreach(\App\Models\Ocupacion::all() as $ocupacion)
                    <option value="{{ $ocupacion->id }}" {{ old('id_ocupacion', $this->user->id_ocupacion) == $ocupacion->id ? 'selected' : '' }}>
                        {{ $ocupacion->nombreOcupacion }}
                    </option>
                @endforeach
            </select>
            @error('id_ocupacion') <span class="text-red-500">{{ $message }}</span> @enderror
        </div>
        {{-- id_poblacion --}}
        <div class="col-span-6 sm:col-span-4">
            ...
            <select wire:model="state.id_poblacion" id="id_poblacion" class="mt-1 block w-full">
                ...
                @foreach(\App\Models\Poblacion::all() as $poblacion)
                    <option value="{{ $poblacion->id }}" {{ old('id_poblacion', $this->user->id_poblacion) == $poblacion->id ? 'selected' : '' }}>
                        {{ $poblacion->nombrePoblacion }}
                    </option>
                @endforeach
            </select>
            @error('id_poblacion') <span class="text-red-500">{{ $message }}</span> @enderror
        </div>
    </x-slot>
